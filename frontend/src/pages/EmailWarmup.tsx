...
import { Flame, Plus, Send, Target, ShieldAlert, Activity, Play, Pause, Upload } from "lucide-react";
...
const EmailWarmup = () => {
  ...
  const fetchData = async () => {
    ...
    const accountsWithStats = warmups?.map(account => {
      const accountSends = allSends?.filter(s => s.inbox_id === account.inbox_id) || [];
      const totalSent = accountSends.length;
      const inboxCount = accountSends.filter(s => s.placement === 'inbox').length;
      const spamCount = accountSends.filter(s => s.placement === 'spam').length;
      const inboxRate = totalSent > 0 ? Math.round((inboxCount / totalSent) * 100) : 0;
    ...
    const inboxCount = allSends?.filter(s => s.placement === 'inbox').length || 0;
    const spamCount = allSends?.filter(s => s.placement === 'spam').length || 0;
    const inboxRate = totalSent > 0 ? Math.round((inboxCount / totalSent) * 100) : 0;

    // Calculate average health score
    const avgHealth = warmups?.reduce((acc, curr) => 
      acc + (Number(curr.inboxes?.health_score) || 0), 0) / (warmups?.length || 1);
  ...
  const handleAddAccount = async (e: React.FormEvent) => {
    ...
    const { data: inbox, error: inboxError } = await supabase
      ...
      .insert({
        ...
        imap_password: formData.imapPassword,
    ...
    if (formData.warmupEnabled) {
      const { error: warmupError } = await supabase
        ...
        .insert({
          ...
          warmup_limit: formData.warmupLimit,
          warmup_increment: formData.warmupIncrement,
          progress_percentage: 0,
        });

      if (warmupError) {
        toast.error("Error creating warmup account");
    ...
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        try {
          ...
          // Create warmup account if enabled
          if (row['Warmup Enabled'] === 'TRUE' || row['Warmup Enabled'] === 'true') {
            const { error: warmupError } = await supabase
              .from('warmup_accounts')
              .insert({
                user_id: user.id,
      ...
      toast.success(`Successfully uploaded ${successCount} accounts${errorCount > 0 ? `, ${errorCount} failed` : ''}`);
      setDialogOpen(false);
      setCsvFile(null);
      fetchData();
    } catch (error) {
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-foreground mb-2">Email Warmup</h1>
            <p className="text-muted-foreground">Gradually increase email sending reputation</p>
          </div>
            ...
            <DialogContent className="max-w-2xl">
              ...
              <Tabs value={uploadMethod} onValueChange={(v) => setUploadMethod(v as "manual" | "csv")}>
                ...
                <TabsContent value="manual">
                  <form onSubmit={handleAddAccount} className="space-y-4 max-h-[60vh] overflow-y-auto px-1">
                    ...
                    <div className="border-t pt-4 mt-4">
                      <h3 className="font-semibold text-sm mb-3">SMTP Settings</h3>
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-2">
                    ...
                    <div className="border-t pt-4 mt-4">
                      <h3 className="font-semibold text-sm mb-3">Warmup Settings</h3>
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          ...
                          <Switch
                            ...
                            onCheckedChange={(checked) => setFormData({ ...formData, warmupEnabled: checked })}
                          />
                        </div>
                        ...
                        {formData.warmupEnabled && (
                          <div className="grid grid-cols-3 gap-4">
                            <div className="space-y-2">
                              ...
                              />
                              <p className="text-xs text-muted-foreground">Starting emails per day</p>
                            </div>
                            <div className="space-y-2">
                              ...
                              <Input
                                ...
                                min="1"
                                max="200"
                                value={formData.warmupLimit}
                                onChange={(e) => setFormData({ ...formData, warmupLimit: parseInt(e.target.value) })}
                              />
                  ...
                  <form onSubmit={handleCsvUpload} className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <div className="text-xs text-muted-foreground space-y-1 mt-2">
                        ...
                        <p>Email, First Name, Last Name, IMAP Username, IMAP Password, IMAP Host, IMAP Port, SMTP Username, SMTP Password, SMTP Host, SMTP Port, Daily Limit, Warmup Enabled, Warmup Limit, Warmup Increment</p>
                      </div>
                    </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card className="border-border">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">Total warmup emails</p>
            </CardContent>
          </Card>
          ...
          <Card className="border-border">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-green-500">{stats.inboxRate}%</div>
              <p className="text-xs text-muted-foreground mt-1">Landing in inbox</p>
            </CardContent>
          </Card>
          ...
          <Card className="border-border">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-red-500">{stats.spamEmails}</div>
          ...
          <Card className="border-border">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">Average across accounts</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="border-border">
          <CardHeader>
            ...
            <CardDescription>Monitor warmup progress for all accounts</CardDescription>
          </CardHeader>
          <CardContent>
            {loading ? (
              ...
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p>No accounts connected yet. Add your first account to start warming up.</p>
              </div>
            ) : (
              <Table>
                <TableHeader>
                    ...
                    <TableRow key={account.id}>
                      ...
                      <TableCell>
                        <span className={`px-2 py-1 rounded text-xs ${
                          account.warmup_status === 'active' ? 'bg-green-500/20 text-green-500' :
                          account.warmup_status === 'paused' ? 'bg-orange-500/20 text-orange-500' :
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <Progress value={Number(account.progress_percentage)} className="w-20 h-2" />
                      ...
                      <TableCell>
                        <span className={`font-semibold ${
                          Number(account.inboxes?.health_score) >= 80 ? 'text-green-500' :
                          Number(account.inboxes?.health_score) >= 60 ? 'text-orange-500' :
                          'text-red-500'
                        }`}>
                      ...
                      <TableCell>
                        <span className={`font-semibold ${
                          account.stats?.inboxRate >= 80 ? 'text-green-500' :
                          account.stats?.inboxRate >= 60 ? 'text-orange-500' :
                          'text-red-500'
                        }`}>
                      ...
                      <TableCell>
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleToggleStatus(account.id, account.warmup_status)}
                        >
