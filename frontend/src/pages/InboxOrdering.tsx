...
const InboxOrdering = () => {
	...
	useEffect(() => {
		...
		console.log('[INBOX-ORDERING] Payment callback triggered', { payment, sessionId });
	...
	const processPendingOrder = async (sessionId: string) => {
		console.log('[INBOX-ORDERING] Processing pending order for session:', sessionId);
		...
		try {
			...
			if (fetchError) {
				console.error('[INBOX-ORDERING] Error fetching pending order:', fetchError);
				throw fetchError;
			}

			if (!pendingOrders || pendingOrders.length === 0) {
				console.error('[INBOX-ORDERING] No pending order found for session:', sessionId);
				toast({
					title: "Order Not Found",
					description: "Could not find your order details. Please contact support.",
					variant: "destructive"
				});
				return;
			}
			...
			toast({
				...
				description: error.message || "Failed to process your order. Please contact support.",
				variant: "destructive"
			});
		} finally {
	...
	const checkUserPackageAndInboxes = async () => {
		...
		} else if (plan.includes('professional') || plan.includes('pro') || plan.includes('premium') || plan.includes('business')) {
			packageName = plan.includes('business') ? 'business' : 'professional';
		} else {
			packageName = 'starter';
		}

		console.log('User subscription plan:', profile?.subscription_plan, '-> Package:', packageName);
	...
	const createDomainsAndNotify = async (orderData: any) => {
		...
		try {
			...
			const domainRecords = orderData.selectedDomains.map((sel: any) => ({
				...
				user_id: user.id,
				status: 'pending',
				notes: orderData.domainForwarding ? `Forward to: ${orderData.websiteUrl}` : null
			}));
			...
			for (const domain of createdDomains || []) {
				...
				for (let i = 0; i < Math.min(orderData.emailAddresses.length, 3); i++) {
					...
					inboxRecords.push({
						...
						user_id: user.id,
						domain_id: domain.id,
						first_name: orderData.useUniformNames ? orderData.firstName : emailAddr.firstName || orderData.firstName,
						last_name: orderData.useUniformNames ? orderData.lastName : emailAddr.lastName || orderData.lastName,
						status: 'active',
						health_score: 0
					});
				}
			...
			const domainNames = orderData.selectedDomains.map((sel: any) => `${sel.brand}${sel.extension}`).join(', ');
			const orderSummary = `Domains: ${domainNames} | Inboxes: ${(createdInboxes || []).length}`;
			...
			if (orderError) {
				console.error('[INBOX-ORDERING] Order record creation error:', orderError);
			...
			const orderNotification = {
				userId: user.id,
				domains: orderData.selectedDomains.map((sel: any) => `${sel.brand}${sel.extension}`),
				inboxes: (createdInboxes || []).map(inbox => {
					const domainName = createdDomains?.find(d => d.id === inbox.domain_id)?.domain_name || '';
					return {
			...
			const { error: notificationError } = await supabase.functions.invoke('send-inbox-order-notification', {
				body: orderNotification
			});

			if (notificationError) {
				console.error('[INBOX-ORDERING] Failed to send order notification:', notificationError);
			} else {
						...
						<Card className="border-border">
							...
							<CardContent className="space-y-4">
								<div className="flex items-start gap-3 p-4 bg-primary/10 border border-primary/20 rounded-lg">
									...
									<div className="text-sm">
										<p className="font-medium text-foreground mb-1">Secure Payment via Stripe</p>
										<p className="text-muted-foreground">
											Your payment will be processed securely through Stripe. You'll be redirected to complete your purchase.
										</p>
									</div>
								</div>
								...
								<div className="flex gap-3 pt-4">
									...
									<Button
										onClick={async () => {
											setIsSubmitting(true);
											try {
												// First, save the pending order to the database
												const { data: { user } } = await supabase.auth.getUser();
												if (!user) throw new Error("You must be logged in");

												console.log('[INBOX-ORDERING] Creating pending order in database');
												...
												const { data: pendingOrder, error: pendingError } = await supabase
													...
													.insert({
														...
														use_uniform_names: useUniformNames,
														same_emails_for_all_domains: sameEmailsForAllDomains,
												...
												if (pendingError) {
													console.error('[INBOX-ORDERING] Error creating pending order:', pendingError);
													throw pendingError;
												}

												console.log('[INBOX-ORDERING] Pending order created:', pendingOrder.id);

												// Now create Stripe checkout with pending order ID
												...
												if (data?.url) {
													...
													// Redirect in same window so Stripe can redirect back properly
